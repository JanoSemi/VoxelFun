...
static func generate_details(c, rng, _ground_height):
	...
	db.create("Diamond")
	var dx = rng.randi_range(0, int(Global.CHUNK_DIMENSION.x))
	var dy = rng.randi_range(1, 10)
	var dz = rng.randi_range(0, int(Global.CHUNK_DIMENSION.z))
	c._set_block_data(dx, dy, dz, db, true, false)
	# Place gold
	var gb = c.BlockData.new()
	gb.create("Gold")
	var gx = rng.randi_range(0, int(Global.CHUNK_DIMENSION.x))
	var gy = rng.randi_range(1, 10)
	var gz = rng.randi_range(0, int(Global.CHUNK_DIMENSION.z))
